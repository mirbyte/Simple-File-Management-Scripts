...
def detect_password_requirement(archive_path: Path) -> bool:
    """
    Properly detects if an archive requires a password by attempting to read file content.
    ...
    try:
        with ArchiveOpener(archive_path, 'r') as archive_ref:
            ...
            # Find the first actual file (not directory) to test
            test_file = None
            for filename in file_list:
                # Skip directories - look for actual files
                if not filename.endswith('/') and not filename.endswith('\\'):
                    test_file = filename
                    break
            
            if not test_file:
                return False  # No files to test, only directories
            
            # Try to read a small portion of the file content
            try:
                if isinstance(archive_ref, zipfile.ZipFile):
                    # For ZIP files, try to read the first few bytes
                    with archive_ref.open(test_file) as f:
                        f.read(1)  # Try to read just 1 byte
                elif isinstance(archive_ref, rarfile.RarFile):
                    # For RAR files, try to read the first few bytes
                    archive_ref.read(test_file, 1)  # Try to read just 1 byte
                
                return False  # If we got here, no password needed
                
            except (RuntimeError, zipfile.BadZipFile) as e:
                # ZIP password errors typically raise RuntimeError
    ...
    except (zipfile.BadZipFile, rarfile.BadRarFile, rarfile.NotRarFile):
        logging.error(f"Archive {archive_path.name} appears corrupt or invalid.")
        return False
    except rarfile.PasswordRequired:
        return True
    except Exception as e:
        logging.warning(f"Could not determine password requirement for {archive_path.name}: {e}")
        return False  # Assume no password needed if we can't determine
        ...
        for src_path in temp_dir_path.rglob('*'):  # rglob gets all files/dirs recursively
            ...
            if dest_path.exists():
                if dest_path.is_dir() and src_path.is_dir():
                    ...
                    if not all_moved and counter > 999:
                        continue  # Continue outer loop if rename failed
...
def process_directory(
    ...
    for item_path in directory.iterdir():
        if item_path.is_file() and get_archive_opener(item_path):  # Check if it's a supported archive file
            ...
            # Use improved password detection
            needs_password = detect_password_requirement(archive_path)
            if needs_password:
